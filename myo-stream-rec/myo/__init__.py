...
def discover_myos(adapter):
    ...
    while time.time() - t < 1.0:  # todo, put a timeout here.
        ...
        if uuid == MyoServiceInfoUuid:  # This is MYO_SERVICE_INFO_UUID # found a myo.
            ...
            myo_details[name] = mac_address_string # this can be wrong if there are multiple myos -- Qichao
    adapter.end_scan()
    return myo_details
...
class Myo(object):
    ...
    def emg_to_csv(self):
        for i, value in enumerate(self.emg_index):
            if value is 0:
                try:
                    self.emg_index[i] = (self.emg_index[i-1] + self.emg_index[i+1]) / 2
                except:
                    self.emg_index[i] += self.emg_index[i-1] - \
                    self.emg_index[i-2] + self.emg_index[i-1]

        e = pd.DataFrame(self.emg_data, index=self.emg_index,
        columns=['emg'+str(i+1) for i in range(8)])

        e.to_csv('./data/emg_'+str(self.address).replace(':',"-")+'.csv')

    def imu_to_csv(self):
        imu = pd.DataFrame(self.imu_data, index=self.imu_index, \
        columns=['quat1', 'quat2', 'quat3', 'quat4', 'acc1', 'acc2', \
